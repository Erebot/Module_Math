...
if (version_compare(phpversion(), '5.3.1', '<')) {
    if (substr(phpversion(), 0, 5) != '5.3.1') {
        ...
        echo "Erebot_Module_Math requires PHP 5.3.1 or newer.
";
        exit -1;
    }
}
...
try {
    ...
    echo "Cannot process Erebot_Module_Math phar:
";
    echo $e->getMessage(), "
";
    exit -1;
}
function Erebot_Module_Math_autoload($class)
{
    $class = str_replace(array('_', '\\'), '/', $class);
    if (file_exists('phar://' . __FILE__ . '/Erebot_Module_Math-0.1.0/php/' . $class . '.php')) {
        include 'phar://' . __FILE__ . '/Erebot_Module_Math-0.1.0/php/' . $class . '.php';
    }
}
spl_autoload_register("Erebot_Module_Math_autoload");
$phar = new Phar(__FILE__);
$sig = $phar->getSignature();
define('Erebot_Module_Math_SIG', $sig['hash']);
define('Erebot_Module_Math_SIGTYPE', $sig['hash_type']);
